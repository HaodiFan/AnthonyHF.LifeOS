...
- `memory-wiki`: `object_type=area-index`, `visibility=private`, `authority=identity/memories/wiki-repo.yml`, `configured_instance=AF-wiki`, `public_mirror=index-only`, `collaboration_mode=private-pr-or-owner-approved-extract`, `lifecycle_state=linked`.
...
- Read `identity/memories/wiki-repo.yml` before assuming which memory wiki is configured.

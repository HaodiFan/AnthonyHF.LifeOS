...
| P029 | 撤回：`Hao Daily Notes` 已确认是实习生材料，不能作为 Anthony PSP/person-model 证据。 | Owner correction 2026-05-30。 | 撤回 |
| P030 | 数据生产和交付中会把问题拆到截图时机、bbox 漂移、脏 OCR、key 判断、磁盘空间、限流、上传/下载速度等可操作变量。 | 外部硬盘 Feishu first read：WebAgent 生产复盘 redacted summary；Daily Notes 部分已排除。 | 中低 |
| P031 | Feishu 材料常混合正文、代码、凭证上下文、客户/项目记录，进入 PSP 前必须先确认作者/上下文并做 redaction-first summary。 | 外部硬盘 Feishu first read 安全边界 + owner correction。 | 高 |

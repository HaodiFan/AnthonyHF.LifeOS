...
人物建模产物默认写入本 repo 下的 `people/<person_id>/`，但 `people/` 必须被 `.gitignore` 忽略，不提交到 Skill 包。openLifeOS / LifeOS 调用时可用 `--lifeos-root <repo>` 写入对应 LifeOS 的 `identity/psp/<person_id>/`。
